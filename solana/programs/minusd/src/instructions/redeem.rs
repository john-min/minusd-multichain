...
use anchor_spl::token::{self, BurnChecked, Mint, Token, TokenAccount, TransferChecked};
...
pub fn handler(ctx: Context<Redeem>, amount: u64) -> Result<()> {
    ...
    // Burn first so a failed vault transfer cannot leave extra MINUSD (tx still atomic).
    token::burn_checked(
        CpiContext::new(
